...
class Language(object):
    ...
    def __init__(self, name=str(), ext=list(), files=list(), com=list(), type="(null)", color="#000000", id=0):
        self.name = name
        self.extensions = ext
        self.files = files
        self.comments = com
...
def load_languages():
    ...
    for key, value in data.items():
        if 'files' not in value:
            value["files"] = []
        lang = Language(key, value['extensions'], value['files'], value['comments'],
                        value['type'], value['color'], value['id'])
...
def save_languages():
    ...
    for lang in languages:
        data[lang.name] = {"type": lang.type, "color": lang.color,
                           "extensions": lang.extensions, "comments": lang.comments, "files": lang.files, "id": lang.id}
...
def run_counter(args):
    print()
    if args.git is True and os.path.isfile(".gitignore"):
        with open(".gitignore", "r") as file:
            tmp = file.readlines()
            if args.count_all == False:
                tmp = [".*"] + tmp
            spec = pathspec.PathSpec.from_lines('gitignore', tmp)
    elif args.count_all == False:
    ...
    if args.recurse is True:
        ...
        for dirpath, dirnames, filenames in os.walk(args.dir):
            for filename in [os.path.join(args.dir, dirpath, f) for f in filenames]:
                if spec.match_file(filename) is False:
                    if args.sub_only is False:
                        files.append(filename)
                    else:
                        if os.path.dirname(filename) != os.getcwd():
                            files.append(filename)
            print("\033[AFiles: {}".format(len(files)))
    else:
        files = list()
        for filename in [os.path.join(args.dir, f) for f in listdir(args.dir)]:
            if os.path.isfile(filename) is True and spec.match_file(filename) is False:
                files.append(filename)
    ...
    table_data[0] = ["Language", "Files", "Lines", "Code Lines",
                     "Comment Lines", "Bytes", "Code Bytes", "Comment Bytes", "Percentage"]
    i = 0
    sort = sort_fmt(args.sort)
    for key, value in data.items():
        table_data[i + 1] = [""] * 9
        table_data[i + 1][0] = " {}\u25cf{} {}".format(color.get_color(
            key.color), color.get_color(color.Color.DEFAULT), key.name)
        ...
        i += 1
    table_data[-1] = ["Total", "{}".format(total[0]), "{}".format(total[1][0]), "{}".format(total[1][1]), "{}".format(
        total[1][2]), "{}".format(total[2][0]), "{}".format(total[2][1]), "{}".format(total[2][2]), "100.0%"]
...
def main():
    ...
    sort_choices = ["lang", "files", "lines", "codelines",
                    "commentlines", "bytes", "codebytes", "commentbytes"]
    parser = argparse.ArgumentParser(
        description="Counts lines/bytes of code in a given directory or file")
    parser.add_argument("-v", dest="verbose",
                        action="store_true", help="Print every file scanned")
    parser.add_argument("-r", dest="recurse", action="store_true",
                        help="Recursively finds all files")
    parser.add_argument("-s", dest="sub_only", action="store_true",
                        help="Only counts files of sub directories `-r` must be set")
    parser.add_argument("-a", dest="count_all", action="store_true",
    ...
    parser.add_argument("--sort", type=str, nargs='?', default='bytes',
                        help="Sorts result by data group", choices=sort_choices)
    parser.add_argument("--list", action="store_true",
                        help="Lists languages in directory")
    parser.add_argument("--add", action="store_true",
                        help="Add new language to database")
    parser.add_argument("dir", metavar="DIR", type=str, nargs='?',
                        default=os.getcwd(), help="Directory to run analysis on")
    group = parser.add_argument_group("Display")
    group.add_argument("--table", action="store_true",
                       help="Displays the table of values")
    group.add_argument("--no-table", action="store_true",
                       help="Hides the table of values")
    ...
    group.add_argument("--zebra", action="store_true",
                       help="Forces zebra line highlighting")
    group.add_argument("--no-zebra", action="store_true",
                       help="Disables zebra line highlighting")
    group.add_argument("--table-cell", action="store_true",
                       help="Draws table with unicode cells")
    group.add_argument("--no-table-cell", action="store_true",
                       help="Does not draw table with unicode cells")
    group.add_argument("--bar-width", type=int, default=int(columns),
